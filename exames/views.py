...
def solicitar_exames(request):
    tipos_exames = TiposExames.objects.filter()
    if request.method == "GET":
        return render(request, "solicitar_exames.html", {'tipos_exames': tipos_exames})
    elif request.method == 'POST':
        exame_id = request.POST.getlist('exames')
        solicitacao_exames = TiposExames.objects.filter(id__in=exame_id) # disponivel=True
...
def fechar_pedido(request):
    exame_id = request.POST.getlist('exames')
    solicitar_exames = TiposExames.objects.filter(id__in = exame_id)
